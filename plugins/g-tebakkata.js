...
`.trim()
    conn.tebakkata[id] = [
        await conn.sendButton(m.chat, caption, wm1, null, [['Bantuan', '.teka']], m, {fromMe: false}),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkata[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, wm1, null, [['Tebak Kata', '.tebakkata']], conn.tebakkata[id][0])
