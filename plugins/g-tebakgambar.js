...
Bonus: ${poin} XP
    `.trim()
  conn.tebakgambar[id] = [
    await conn.sendButtonImg(m.chat, caption, wm1, json.img, [['Bantuan', '.hint']], m)
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.tebakgambar[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, wm1, null, [['Tebak Gambar', '.tebakgambar']], conn.tebakgambar[id][0])
