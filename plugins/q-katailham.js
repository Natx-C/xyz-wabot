let handler = async(m) => {
let ilhamm = pickRandom(global.ilham)
    const tytyd = [
  {buttonId: '/katailham', buttonText: {displayText: 'Ilham!!!!'}, type: 1},
]

const kntl = {
    location: { jpegThumbnail: await(await require('node-fetch')(fla + 'Kata Ilham')).buffer()},
    text: ilhamm,
    footer: wm,
    buttons: tytyd,
    headerType: 6
