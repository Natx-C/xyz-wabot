...
`.trim()
    conn.tebakkabupaten[id] = [
        await conn.sendButtonImg(m.chat, caption, wm1, json.url, [['Bantuan', '.teka']], m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkabupaten[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.title}*`, wm1, null, [['Tebak Kabupaten', '.tebakkabupaten']], conn.tebakkabupaten[id][0])
