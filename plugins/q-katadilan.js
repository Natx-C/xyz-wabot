let handler = async(m) => {
let katadilann = pickRandom(global.katadilan)
    const tytyd = [
  {buttonId: '/katadilan', buttonText: {displayText: 'Kata Dilan!!!!'}, type: 1},
]

const kntl = {
    location: { jpegThumbnail: await(await require('node-fetch')(fla + 'Katadilan')).buffer()},
    text: katadilann,
    footer: wm,
    buttons: tytyd,
    headerType: 6
