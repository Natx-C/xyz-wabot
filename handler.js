...
Aktif Selama: ${uptime}`
        ...
        if (settings) {
          ...
          autogetmsg: true,
            ...
            if (opts['queque'] && m.text) {
                sock.msgqueque.push(m.id || m.key.id)
                await delay(sock.msgqueque.length * 1000)
            }
            ...
            let bot = (m.isGroup ? participants.find(u => conn.decodeJid(u.id) == sock.user.jid) : {}) || {} // Your Data
            ...
            for (let name in global.plugins) {
                ...
                if (typeof plugin.before === 'function') if (await plugin.before.call(sock, m, {
                    match,
                    conn: sock,
                    participants,
                    groupMetadata,
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                        let chat = global.db.data.chats[m.chat]
                        let user = global.db.data.users[m.sender]
                        if (name != 'unbanchat.js' && chat && chat.isBanned) return // Except sock
        ...
        conn.sendPresenceUpdate('composing', m.chat) 
            ...
            // try {
            //     require('./lib/print')(m, sock)
            // } catch (e) {
            //     console.log(m, m.quoted, e)
            // }
            if (opts['autoread']) await sock.chatRead(m.chat, m.isGroup ? m.sender : undefined, m.id || m.key.id).catch(() => { })
            let quequeIndex = sock.msgqueque.indexOf(m.id || m.key.id)
    ...
    async participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            //sock.sendFile(id, pp, 'pp.jpg', text, null, false, {
                                //contextInfo: {
                                    //mentionedJid: [user]
                                //}
                            //})
                            sock.sendTemplateButtonLoc(id, text, wm, await(await fetch(pp)).buffer(), action === 'add' ? 'Welcome' : 'Goodbye', 'irwanx', m)
                        }
                    }
                }
                break
            ...
            case 'demote':
                if (!text) text = (chat.sDemote || sock.sdemote || conn.sdemote || '@user ```is no longer Admin```')
                text = text.replace('@user', '@' + participants[0].split('@')[0])
                if (chat.detect) sock.sendMessage(id, text, MessageType.extendedText, {
                    contextInfo: {
...
`.trim(), msg, {
        ...
        sock.copyNForward(msg.key.remoteJid, msg).catch(e => console.log(e, msg))
    }
}
