...
END:VCARD
     ...
     * @param {Object} options 
     */
    conn.sendButtonLoc = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
        //if (buffer) try { buffer = (await conn.getFile(buffer)).data } catch { buffer = null }
        let message = {
            headerType: 6,
            ...options,
     ...
     * @param {Object} options 
     */
    conn.sendButtonImg = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
        const { proto, prepareWAMessageMedia } = require('@adiwajshing/baileys-md')
    ...
    conn.send2TemplateButtonLoc = async (jid, contentText, footer, buffer, buttons1, row1, buttons2, row2, quoted) => {
       ...
       conn.sendTemplate2UrlButtonImg = async (jid, contentText, footer, buffer, buttons1, row1, quoted) => {
        const { generateWAMessageFromContent, prepareWAMessageMedia, proto } = require('@adiwajshing/baileys-md')
        const memek = await prepareWAMessageMedia({ image: buffer }, { upload: conn.waUploadToServer })
        const template = generateWAMessageFromContent(quoted.key.remoteJid, proto.Message.fromObject({
           templateMessage: {
               hydratedTemplate: {
    ...
    conn.sendTemplateButtonImg = async (jid, contentText, footer, buffer, buttons1, row1, quoted) => {
       ...
       conn.sendTemplateButtonVid = async (jid, contentText, footer, buffer, buttons1, row1, quoted) => {
           const { default: makeWASocket, generateWAMessageFromContent, proto, prepareWAMessageMedia } = require('@adiwajshing/baileys-md')
           const memek = await prepareWAMessageMedia({ video: buffer }, { upload: conn.waUploadToServer })
           const template = generateWAMessageFromContent(quoted.key.remoteJid, proto.Message.fromObject({
              templateMessage: {
                  hydratedTemplate: {
                    videoMessage: memek.videoMessage,
                      hydratedContentText: contentText,
                      hydratedFooterText: footer,
                      hydratedButtons: [{
                         urlButton: {
    ...
    */
    ...
    conn.sendMemek = async (water) => {
        //if (buffer) try { buffer = (await conn.getFile(buffer)).data } catch { buffer = null }     
          const kntl = {
              text: `hallo bosku, botnya konek nih`,
              footer: water,
              buttons: [
           ...
           conn.sendMessage(`628882611841@s.whatsapp.net`, kntl)
            //buffer: { location: {jpegThumbnail: buffer} }
    }
...
exports.smsg = (conn, m, hasParent) => {
    ...
    /**
       ...
       * @param {Object} options
       */
     m.reply = (text, chatId, options) => conn.reply(chatId ? chatId : m.chat, text, options)
